...
function CustomerDetail({ customerId, onBack }) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {/* Customer header */}
      <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-6 mb-6">
        <div className="flex gap-6">
          <div className="flex-shrink-0">
            {customer.face_image ? (
              <img
                ...
                alt="Face"
                className="w-40 h-40 object-cover rounded-xl bg-slate-100"
              />
            ) : (
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500 font-mono mb-2">{customer.customer_id}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Spoof Score</p>
        ...
        <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Deepfake Score</p>
          <p className="text-2xl font-bold text-slate-900">
        ...
        <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Pipeline Time</p>
          <p className="text-2xl font-bold text-slate-900">
      ...
      {Object.keys(auditDetails).length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          {auditDetails.spoof_details && (
            <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Spoof Detection Details</h3>
              <div className="space-y-1.5">
                {Object.entries(auditDetails.spoof_details).map(([key, value]) => (
                  <div key={key} className="flex justify-between text-xs">
                    <span className="text-slate-500">{key}</span>
                    <span className="text-slate-900 font-mono">{formatValue(value)}</span>
                  </div>
                ))}
                {auditDetails.motion_penalty != null && (
                  <div className="flex justify-between text-xs pt-1 border-t border-slate-100">
                    <span className="text-slate-500">motion_penalty</span>
                    <span className="text-slate-900 font-mono">{auditDetails.motion_penalty}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {(auditDetails.deepfake_ml || auditDetails.deepfake_frequency) && (
            <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Deepfake Detection Details</h3>
              <div className="space-y-1.5">
                {auditDetails.deepfake_method && (
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">method</span>
                    <span className="text-slate-900 font-mono text-right">{auditDetails.deepfake_method}</span>
                  </div>
                )}
                {auditDetails.deepfake_ml && Object.entries(auditDetails.deepfake_ml).map(([key, value]) => (
                  <div key={key} className="flex justify-between text-xs">
                    <span className="text-slate-500">ml.{key}</span>
                    <span className="text-slate-900 font-mono">{formatValue(value)}</span>
                  </div>
                ))}
                {auditDetails.deepfake_frequency && Object.entries(auditDetails.deepfake_frequency).map(([key, value]) => (
                  <div key={key} className="flex justify-between text-xs">
                    <span className="text-slate-500">freq.{key}</span>
                    <span className="text-slate-900 font-mono">{formatValue(value)}</span>
                  </div>
                ))}
                {auditDetails.deepfake_critical_flag && (
                  <div className="flex justify-between text-xs pt-1 border-t border-slate-100">
                    <span className="text-slate-500">critical_flag</span>
          ...
          {auditDetails.motion_analysis && (
            <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Motion Analysis</h3>
              <div className="space-y-1.5">
          ...
          {auditDetails.risk_breakdown && (
            <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Risk Score Breakdown</h3>
              <div className="space-y-1.5">
                ...
                <div className="pt-1 border-t border-slate-100">
                  {Object.entries(auditDetails.risk_breakdown).map(([key, value]) => (
                    <div key={key} className="flex justify-between text-xs">
                      <span className="text-slate-500">{key}</span>
                      <span className="text-slate-900 font-mono text-right max-w-xs truncate">{formatValue(value)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {auditDetails.flags.map((flag, i) => (
                  <span key={i} className="px-2 py-1 rounded-md bg-red-50 text-red-600 text-xs">{flag}</span>
                ))}
              </div>
            </div>
          )}
      ...
      {/* Audit trail */}
      <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm">
              ...
              <div key={entry.id} className="px-6 py-4">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${getResultBadge(entry.result)}`}>
                      {entry.result}
                    </span>
                    <span className="text-sm text-slate-600">{entry.action}</span>
                  </div>
                ...
                {entry.ip_address && (
                  <p className="text-xs text-slate-400 mt-1">IP: {entry.ip_address}</p>
                )}
              </div>
            ))}
          </div>
