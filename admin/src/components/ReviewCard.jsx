    ...
    <div className="bg-white rounded-xl ring-1 ring-slate-200/60 shadow-sm overflow-hidden">
      ...
      <div className="p-5">
            ...
            {review.flags && review.flags.length > 0 && (
              <div className="flex gap-1 flex-wrap">
                {review.flags.map((flag, i) => (
                  <span key={i} className="px-1.5 py-0.5 rounded bg-red-50 text-red-600 text-[11px]">{flag}</span>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          {/* New registration */}
          <div>
            <p className="text-[11px] font-medium text-slate-400 mb-2 uppercase tracking-wider">New Registration</p>
          ...
          {/* Matched existing */}
          <div>
            <p className="text-[11px] font-medium text-slate-400 mb-2 uppercase tracking-wider">Existing Match</p>
        ...
        {isPending && showNotes && (
          <div className="mb-4">
            <textarea
              ...
              className="w-full p-3 ring-1 ring-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-400 transition-shadow resize-none"
              rows={2}
            />
          </div>
        )}
          ...
          <>
            <div className="flex gap-2.5">
              <button
                ...
                className="flex-1 py-2 px-4 bg-emerald-600 text-white rounded-lg text-[13px] font-medium hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 transition-colors"
              >
              ...
              <button
                ...
                disabled={isProcessing}
                className="flex-1 py-2 px-4 bg-red-600 text-white rounded-lg text-[13px] font-medium hover:bg-red-700 disabled:bg-slate-200 disabled:text-slate-400 transition-colors"
              >
            ...
            {showNotes && (
              <button
                ...
                className="w-full mt-2 py-1.5 text-[13px] text-slate-400 hover:text-slate-600 transition-colors"
              >
                Cancel
              </button>
            )}
